...
%                     results.inc_rlsc_norec.teAcc(k,j,q,lidx) = currAcc;
    ...
    %                 Ytr_tmp = [Ytr_tmp ; Ytr_imbal(q,:)];
                    ...
                    for lidx = 1:numel(lrng)
                        ...
                        results(kk).inc_rlsc_yesrec.valAcc(k,imbClass,q,lidx) = currAcc;
...
%                     end
                    ...
                    results(kk).inc_rlsc_yesrec.testAccBuf(k,imbClass,q) = currAcc;
                    results(kk).inc_rlsc_yesrec.testCM(k,imbClass,q,:,:) = CM;
                    results(kk).inc_rlsc_yesrec.bestValAccBuf(k,imbClass,q) = bestAcc;
                    results(kk).inc_rlsc_yesrec.bestCMBuf(k,imbClass,q,:,:) = bestCM;
                    results(kk).inc_rlsc_yesrec.bestLambdaBuf(k,imbClass,q) = lstar;                
...
%         end
        ...
        for kk = 1: numAlpha
            subplot(2,5,kk)
            h1 = bandplot(1:min(maxiter,out(c,2)),squeeze(results(1).inc_rlsc_norec.bestCMBuf(:,c,1:min(maxiter,out(c,2)), c, c)), ...
                'r' , 0.1 , 0 , 1 , '-');
        ...
        for kk = 1: numAlpha
            ...
            a1 = squeeze(results(1).inc_rlsc_norec.bestValAccBuf(:,c,1:min(maxiter,out(c,2))));
            b1 = squeeze(results(1).inc_rlsc_norec.bestCMBuf(:,c,1:min(maxiter,out(c,2)), c, c));
            c1 = (t*a1 - b1) / (t-1);
